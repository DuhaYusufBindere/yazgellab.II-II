...
class GRUClassifier(nn.Module):
    ...
    def __init__(self, input_size, hidden_size=None, num_layers=2, dropout=0.2, config=None):
        ...
        if hidden_size is None:
            # Karşılaştırma kolaylığı açısından LSTM ile aynı hidden_size değerini paylaşıyoruz
            hidden_size = config.get("deep_learning", {}).get("lstm_hidden_size", 64)
            
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        
        # GRU Katmanı (Giriş boyutu: [batch_size, sequence_length, input_size])
        self.gru = nn.GRU(
        ...
        # Son zaman adımındaki GRU çıkışını ikili sınıflandırma skoru (logit) olarak eşler.
    ...
    def forward(self, x):
        ...
        """
        # gru_out boyutu: [batch_size, sequence_length, hidden_size]
